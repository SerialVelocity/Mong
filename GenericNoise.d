...
static pure class GenericNoise {
	
	static const immutable(int) X_NOISE_GEN = 1619;
	static const immutable(int) Y_NOISE_GEN = 31337;
	static const immutable(int) Z_NOISE_GEN = 6971;
	static const immutable(int) SEED_NOISE_GEN = 1013;
	static const immutable(int) SHIFT_NOISE_GEN = 8;
	
	static pure float SCurve3 (float a)
	{
		return (a * a * (3.0f - 2.0f * a));
	}
	
	static pure float SCurve5 (float a)
	{
		float a3 = a * a * a;
		float a4 = a3 * a;
		float a5 = a4 * a;
	...
	static pure float LinearInterp (float n0, float n1, float a)
	{
		return ((1.0 - a) * n0) + (a * n1);
	}
	
	static pure float GradientCoherentNoise3D (float x, float y, float z, int seed, NoiseQuality quality) {
		// Create a unit-length cube aligned along an integer boundary.  This cube
		// surrounds the input point.
		int x0 = cast(int)x - (x > 0.0f ? 0 : 1);
		int x1 = x0 + 1;
		int y0 = cast(int)y - (y > 0.0f ? 0 : 1);
		int y1 = y0 + 1;
		int z0 = cast(int)z - (z > 0.0f ? 0 : 1);
		int z1 = z0 + 1;
	...
	static pure float GradientNoise3D (float fx, float fy, float fz, int ix, int iy, int iz, int seed) {
	...
	static const immutable(float) g_randomVectors[256 * 4] = [
